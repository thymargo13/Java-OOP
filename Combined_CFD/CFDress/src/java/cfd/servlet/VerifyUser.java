 ...
 * @author Daniel
 */
@WebServlet(name = "VerifyUser", urlPatterns = {"/cms/VerifyUser"})
public class VerifyUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            db.verifyCustomer(custId);//call custDB to update status.
            request.setAttribute("id", custId);
            request.getRequestDispatcher("ViewUser").forward(request, response);

            /*
            RequestDispatcher rd;
            String targetURL = "verification.jsp";
            rd = getServletContext().getRequestDispatcher("/CMS/" + targetURL);
